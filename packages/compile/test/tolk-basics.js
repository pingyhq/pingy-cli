...
describe('readCompiled', () => {
  ...
  it('should throw when reading a file that does not exist', () =>
    expect(tolk.read(getPath('does-not-exist.txt')), 'when rejected', 'to satisfy', {
      code: 'ENOENT',
      path: expect.it('to contain', Path.join('fixtures/source/does-not-exist.txt')),
      message: expect
        .it('to contain', Path.join('fixtures/source/does-not-exist.txt'))
        .and('to contain', 'ENOENT'),
    }));
  ...
  it('should throw when compiling a file that does not exist', () =>
    expect(tolk.read(getPath('does-not-exist.scss')), 'when rejected', 'to satisfy', {
      code: 'ENOENT',
      path: expect.it('to contain', Path.join('fixtures/source/does-not-exist.scss')),
      message: expect
        .it('to contain', Path.join('fixtures/source/does-not-exist.scss'))
        .and('to contain', 'ENOENT'),
    }));
  ...
  it('should throw when compiling a file with syntax errors', () =>
    expect(tolk.read(getPath('scss/syntaxerror.scss')), 'when rejected', 'to satisfy', {
      status: 1,
      file: expect.it('to contain', Path.join('fixtures/source/scss/syntaxerror.scss')),
      line: 2,
      column: 3,
